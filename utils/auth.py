...
def create_user(username: str, password: str, totp_enabled: bool = True) -> Tuple[bool, Optional[str]]:
    """Create user. Returns (success, totp_secret_or_None)."""
    users = load_users()
...
def require_auth():
    """Call at top of each page. Stops page if not authenticated."""
    if not st.session_state.get("authenticated"):
        st.warning("請先登入，點選左側選單回到首頁。")
        st.stop()


def logout():
    for key in ["authenticated", "username", "portfolio_data", "prices", "usd_twd"]:
